...
class LongShort {
  ...
  allStocks: { name: string; pc: number }[];
  ...
  constructor(keyId: string, secretKey: string, paper: boolean) {
    ...
    this.allStocks = allStocks.map(name => ({ name, pc: 0 }));
            ...
            resp.forEach(position => {
              promClose.push(
                new Promise(async (resolve, reject) => {
                  ...
                  const quantity = Math.abs(parseInt(position.qty, 10));
  ...
  async rerank() {
    ...
    for (const stock of this.allStocks.entries()) {
    }
